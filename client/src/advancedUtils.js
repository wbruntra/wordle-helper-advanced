import { wordsAtOrBelowLimit } from './scorers.js'
...
const evaluateGuess = (canonicalGuess, canonicalAnswer) => {
...
export const getBestHitFromFullList = (
  filteredList,
  allWords,
  { limit = 1, verbose = false, strictness_proportion = 1},
) => {
 ...
 */
export const analyzeGuess = (guess, key, remainingWords) => {
  ...
  const analysis = {
    ...
    minBinSize: _.min(binSizes),
    maxBinSize: _.max(binSizes),
  ...
  if (includeMatches) {
    analysis.binBreakdown = Object.entries(bins).map(([evaluation, words]) => ({
      evaluation,
      count: Array.isArray(words) ? words.length : words,
 ...
 */
export const findOptimalGuesses = (wordList, topN = 1, options = {}) => {
